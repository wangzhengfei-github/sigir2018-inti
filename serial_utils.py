...
def change_traffic_light(configuration):
    xmlfile = ElementTree()
    xmlfile.parse('tmp/di-tech.net.xml')
    ...
    xmlfile.write('tmp/di-tech.net.xml', encoding='utf-8', xml_declaration=True)


def calculate_waitsteps():
    total_loss = 0.0
    n_vehicles = 0
    dom = minidom.parse('tmp/tripinfo.xml')
